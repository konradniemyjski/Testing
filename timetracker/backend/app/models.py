"""Fixed models.py with corrected imports."""
...
class CateringCompany(Base, TimestampMixin):
    ...
    # Relationships
    worklogs: Mapped[list["WorkLog"]] = relationship(
        back_populates="catering_company"
    )


class AccommodationCompany(Base, TimestampMixin):
    """Accommodation company model."""
    ...
    # Relationships
    worklogs: Mapped[list["WorkLog"]] = relationship(
        back_populates="accommodation_company"
    )
...
class TeamMember(Base, TimestampMixin):
    ...
    name: Mapped[str] = mapped_column(String(255), unique=True, nullable=False)
    role: Mapped[str] = mapped_column(
        String(50), nullable=False, default="Pracownik"
    )
    team_id: Mapped[int | None] = mapped_column(
...
class WorkLog(Base, TimestampMixin):
    ...
    accommodation_company_id: Mapped[int | None] = mapped_column(
        ForeignKey("accommodation_companies.id", ondelete="SET NULL"),
        nullable=True,
    )
    ...
    catering_company: Mapped["CateringCompany | None"] = relationship(
        back_populates="worklogs"
    )
